...
export class CognitiveOrchestrator {
    ...
    public async initialize(): Promise<void> {
        ...
        window.addEventListener('nexus-force-evolution', this.forceEvolutionCycle);
    }
    
    private forceEvolutionCycle = () => {
        console.log('[NEXUS-CORE] Manual evolution cycle triggered by user.');
        this.dispatchThought('Ciclo de evolução forçado manualmente.', 'symbolic_log');
        ...
        try {
            ...
            if (agentResponse) {
                ...
                if (agentResponse.type !== 'status') {
                    isSpeaking = true;
                    setStatus('SPEAKING');
                    speak(agentResponse.text, () => setStatus('IDLE'));
                }
            }
    ...
    public dispose(): void {
        ...
        window.removeEventListener('nexus-force-evolution', this.forceEvolutionCycle);
    }
    ...
    private startMaintenanceEngine(): void {
        ...
        this.maintenanceInterval = window.setInterval(() => {
            ...
            if (isIdle) {
                this.touchHeartbeat(); // Reset timer to avoid running it every minute after it starts
                this.maintenanceAgent.runMaintenanceCycle(this.presentCodeProposal.bind(this));
            }
        }, 1 * 60 * 1000);
    }
