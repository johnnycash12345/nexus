...
import { getAllConcepts, deleteConcept } from '../services/memoryService';
  ...
  const renderTabContent = () => {
    switch (activeTab) {
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto pr-2">
                {concepts.length > 0 ? concepts.map(c => (
                    <div key={c.name} className="bg-gray-700 p-3 rounded-md">
                        ...
                        {c.definition && <p className="text-xs italic text-gray-300 mt-1">"{c.definition}"</p>}
                    </div>
      ...
      case 'behavior':
        return (
            <div>
                <h3 className="text-lg font-semibold text-cyan-300 mb-4">Comportamento</h3>
                <div className="space-y-4">
                    <label className="flex items-center justify-between p-3 bg-gray-700 rounded-md cursor-pointer">
                        <div>
                            <p className="font-medium text-white">Diário de Aprendizado</p>
                            <p className="text-sm text-gray-400">Gerar um resumo diário do que aprendeu.</p>
                        </div>
                        <input type="checkbox" name="enableDiary" checked={settings.behavior.enableDiary} onChange={handleBehaviorChange} className="toggle-checkbox" />
                    </label>
                    <label className="flex items-center justify-between p-3 bg-gray-700 rounded-md cursor-pointer">
                        <div>
                            <p className="font-medium text-white">Curiosidade Proativa</p>
                            <p className="text-sm text-gray-400">Fazer perguntas para aprender mais.</p>
                        </div>
